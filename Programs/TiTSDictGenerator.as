package  {
	...
	public class TiTSDictGenerator extends MovieClip {
		...
		public function TiTSDictGenerator() {
			createHipsAry();
			createHairAry();
			createAssAry();
			createCockAry();
				...
				for (j = 0; j < typeArray[i][0][0].length; j++){
					...
					for (k = 0; k < typeArray[i][0][1].length; k++){
						...
						for (l = 0; l < typeArray[i][0][2].length; l++){
							...
							if ((typeArray[i][0][0][j] + typeArray[i][0][1][k] + typeArray[i][0][2][l]).toString().indexOf("*") == -1){
								if (className == ""){
									classDict[key.toString()] = typeArray[i][0][0][j] + typeArray[i][0][1][k] + typeArray[i][0][2][l];
								}else{
									classDict[key.toString()] = className;
								}
								text += ('this["' + key + '"] = "' + classDict[key.toString()] + '";') + "\r\n";
							}
